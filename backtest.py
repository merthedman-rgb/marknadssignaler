...
def naive_baseline(close: pd.Series) -> tuple[float, float]:
    """Om du alltid hade gissat 'uppåt' rakt av, varje dag, oavsett signal -
    vad hade träffsäkerheten och snittavkastningen blivit över samma
    FORWARD_DAYS-fönster? Detta är den riktiga kontrollen för om reglerna
    tillför något utöver att marknaden bara gick upp överlag under perioden."""
    n = len(close)
    returns = [
        (close.iloc[i + FORWARD_DAYS] - close.iloc[i]) / close.iloc[i] * 100
...
def summarize(label: str, events: list[SignalEvent], baseline_return_pct: float,
              naive_win_rate: float, naive_avg_return: float) -> None:
    print(f"\n=== {label} ===")
    if not events:
    ...
    print(f"\n--- Naiv baseline (alltid gissa 'uppåt', ingen strategi alls) ---")
    print(f"Naiv träffsäkerhet:    {naive_win_rate:.1f}%")
    print(f"Naiv snittavkastning:  {naive_avg_return:+.2f}%  (över {FORWARD_DAYS} handelsdagar)")

    edge_win = win_rate - naive_win_rate
    edge_ret = avg_return - naive_avg_return
    print(f"\nReglernas edge över naiv gissning: {edge_win:+.1f} procentenheter träffsäkerhet, "
          f"{edge_ret:+.2f}%-poäng snittavkastning")
    if edge_ret <= 0:
        print("⚠ Reglerna presterar INTE bättre än att bara alltid gissa uppåt under denna period.")
    elif edge_win <= 0:
        print("⚠ Reglerna vinner oftare per krona, men träffar inte oftare än naiv gissning — blandad bild.")
    else:
        print("✓ Reglerna slår den naiva baselinen på båda måtten under denna period.")
...
def main():
    ...
    for key, meta in ASSETS.items():
        ...
        naive_win_rate, naive_avg_return = naive_baseline(close)

        events = backtest_asset(meta["ticker"])
        summarize(meta["label"], events, float(baseline), naive_win_rate, naive_avg_return)
