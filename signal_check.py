...
def fetch_live_gold_spot(prev_entry: dict) -> tuple[float | None, str | None]:
    ...
    prev_checked = prev_entry.get("spot_checked_utc")
    now = pd.Timestamp.now("UTC")

    if prev_checked:
        try:
            prev_dt = pd.Timestamp(prev_checked).tz_localize("UTC")
...
def run(fetch_fn=fetch_close_series) -> list[tuple]:
    ...
    for key, meta in ASSETS.items():
        ...
        now_str = pd.Timestamp.now("UTC").strftime("%Y-%m-%d %H:%M")
        ...
        if key == "guld":
            ...
            state[key]["spot_checked_utc"] = spot_checked
